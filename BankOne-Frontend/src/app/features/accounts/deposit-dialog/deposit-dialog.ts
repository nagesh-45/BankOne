...
export class DepositDialog {
  private readonly dialogRef = inject(MatDialogRef<DepositDialog, boolean>);
  private readonly data = inject<DepositDialogData>(MAT_DIALOG_DATA, {
    optional: true
  });
  ...
  submit(): void {
    ...
    if (!Number.isFinite(amount) || amount <= 0) {
      this.notification.error('Enter a deposit amount greater than zero');
      return;
    }
    ...
    this.accountService
      .deposit(account.accountId, amount)
      .pipe(finalize(() => this.saving.set(false)))
      .subscribe({
        next: () => {
          this.notification.success('Deposit successful');
          this.dialogRef.close(true);
        },
        error: (error) => {
          this.notification.error(apiErrorMessage(error, 'Deposit failed'));
        }
      });
  }
}
